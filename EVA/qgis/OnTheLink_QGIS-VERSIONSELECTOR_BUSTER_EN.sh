...
selection=$(dialog \
    ...
    --ok-label "Continue" \
    --cancel-label "Cancel" \
    --menu "OnTheLink_QGIS-VERSIONSELECTOR | EVA-BUSTER | All Options" $HEIGHT $WIDTH 0 \
...
case $selection in
    ...
    4 )
		...
		tput sgr0
		cd $STARTDIR
		sudo rm -rf OnTheLink_QGIS-VERSIONSELECTOR_BUSTER_EN.sh 2> /dev/null | exec 1> /dev/tty
